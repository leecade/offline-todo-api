...
function todos(db) {
  ...
  router.route('/:created')
    ...
    .get(function(req, res) {
      db.findOne({ _id: parseInt(req.param('created'), 10), deleted: { $ne: true } }, function(err, doc) {
    ...
    .put(function(req, res) {
      if (req.body.text && req.body.updated) {
        db.findOne({ _id: parseInt(req.param('created'), 10) }, function(err, doc) {
          if (doc && doc.deleted) {
            res.status(410).end();
          } else if (doc && doc.updated === req.body.updated && doc.text === req.body.text) {
            res.status(204).end();
            ...
            db.insert({ _id: parseInt(req.param('created'), 10), text: req.body.text, updated: req.body.updated }, function(err, newDoc) {
              eventEmitter.emit('update');
              res.status(201).end();
            });
          }
      ...
      if (req.body.text && req.body.updated) {
        db.findOne({ _id: parseInt(req.param('created')) }, function(err, doc) {
          if (doc && (doc.updated > req.body.updated || doc.text !== req.body.text)) {
            ...
            db.update({ _id: parseInt(req.param('created'), 10) }, { text: req.body.text, updated: req.body.updated, deleted: true }, function(err, removed) {
              eventEmitter.emit('update');
              res.status(202).end();
            });
          }
